...
@onready var current_cost = $Cost.value
...
func _ready() -> void:
	...
	if new_texture != null:
		texture = new_texture
		print("TextureRect texture updated successfully to: ", GlobalVar.current_hero)
	else:
...
# Function to dynamically change TextureRect texture
func change_texture_rect_texture(new_texture_path: String) -> bool:
	# Attempt to load new texture
	var loaded_texture: Texture2D = load(new_texture_path)

	if loaded_texture != null:
		texture = loaded_texture
		print("TextureRect texture dynamically updated to: ", new_texture_path)
		return true
	else:
...
func update_hero_texture() -> void:
	...
	change_texture_rect_texture(texture_path)
